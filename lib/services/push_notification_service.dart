...
class PushNotificationService {
  ...
  static const Duration _initStepTimeout = Duration(seconds: 8);
  ...
  Future<void> init() async {
    ...
    try {
      await Firebase.initializeApp().timeout(_initStepTimeout);
    } catch (e) {
    ...
    try {
      ...
      debugPrint('[Push] Foreground presentation options skipped: $e');
    }

    try {
      await messaging.setAutoInitEnabled(true).timeout(_initStepTimeout);
    } catch (e) {
      debugPrint('[Push] Auto-init setup skipped: $e');
    }
    ...
    try {
      initialMessage = await messaging.getInitialMessage().timeout(_initStepTimeout);
    } catch (e) {
      debugPrint('[Push] Initial message fetch skipped: $e');
    }
  ...
  Future<void> syncTokenWithServer() async {
    if (!_initialized) {
      try {
        await init().timeout(_tokenTimeout);
      } catch (e) {
        debugPrint('[Push] Init did not complete before token sync: $e');
        return;
      }
      if (!_initialized) return;
    }
    ...
    if (defaultTargetPlatform == TargetPlatform.iOS ||
      ...
      if (apnsToken == null || apnsToken.isEmpty) {
        debugPrint('[Push] APNs token not available yet; skipping token sync.');
        return;
      }
    }
    ...
    await _api
        .updatePushToken(token: token, platform: _platformName())
        .timeout(_tokenTimeout);
  }

  Future<String?> _waitForApnsToken() async {
    final maxAttempts = _tokenTimeout.inMilliseconds ~/ _apnsPollInterval.inMilliseconds;
