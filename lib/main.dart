...
class _PgpChatAppState extends State<PgpChatApp> {
  ...
  static const Duration _pushInitTimeout = Duration(seconds: 12);
  ...
  Future<void> _initializeServices() async {
    // Run startup services in background so first frame is never blocked.
    try {
      await ApiService().setBaseUrl('http://93.127.129.90:3000/api');
    } catch (e) {
      debugPrint('[Bootstrap] Failed to set API base URL: $e');
    }

    try {
      await PinService().init();
    } catch (e) {
      debugPrint('[Bootstrap] Failed to initialize PIN service: $e');
    }

    try {
      await PushNotificationService().init().timeout(_pushInitTimeout);
    } catch (e) {
      debugPrint('[Bootstrap] Push service init did not complete: $e');
    }
  }
